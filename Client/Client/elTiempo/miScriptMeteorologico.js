...
function ajaxCheckWeather() {
  ...
  // Mostrar latitud y longitud en el documento HTML
  muestraLatitud(longitud);
  muestraLongitud(latitud);
  ...
  fetch(`https://api.openweathermap.org/data/2.5/weather?q=${ubicacion}&appid=2123b15abf5dbccb4b78d19ccea8dd7d`)
    .then(response => response.json())
    .then(data => {
      const latitud = data.coord.lat;
      const longitud = data.coord.lon;
      muestraLatitud(latitud);
      muestraLongitud(longitud);
    })
